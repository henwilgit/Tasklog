...
def update_entry(entry_id):
    ...
    with get_db() as conn:
        ...
        # Recalculate priority if type or date changed
        ...
        conn.execute(
            'UPDATE entries SET date = ?, type = ?, text = ?, priority = ?, updated_at = ? WHERE id = ?',
            (new_date, new_type, data['text'], new_priority, now, entry_id)
        )
        conn.commit()
